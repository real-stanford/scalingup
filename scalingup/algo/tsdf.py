...
from scalingup.utils.core import Observation

from scalingup.algo.virtual_grid import VirtualGrid
...
def obs_to_tsdf_vol(
    obs: Observation, virtual_grid: VirtualGrid, use_gpu: bool
) -> TSDFVolume:
    vol_bnds = np.array(virtual_grid.scene_bounds).T
    voxel_sizes = (vol_bnds[:, 1] - vol_bnds[:, 0]) / virtual_grid.grid_shape
    assert np.allclose(np.diff(voxel_sizes), 0), f"voxel size ({voxel_sizes}) not cuboid"
    ...
    for key, view in obs.images.items():
        logging.debug(f"{key}: {view.obs_dim}")
        tsdf.integrate(
            color_im=view.rgb,
            depth_im=view.depth,
            cam_intr=view.intrinsic,
            cam_pose=view.pose_matrix,
        )
    return tsdf
