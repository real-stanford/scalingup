...
class TableTopFR5MujocoEnv(MujocoFR5EnvFromObjConfigList):
    def __init__(
        self,
        table_asset_path: str = "scalingup/environment/mujoco/assets/custom/table.xml",
        table_length: float = 1.165,
    ...
    def get_are_objs_on_ground(
        state: EnvState,
        non_obj_keywords: FrozenSet[str] = frozenset({"_area", "table", "world", "fr5"}),
        verbose: bool = False,
    ) -> Dict[str, bool]:
        objects = {
            obj_name: obj_state
    ...
    def get_state(self) -> EnvState:
        state = super().get_state()
        if self.end_episode_on_drop and any(
            TableTopFR5MujocoEnv.get_are_objs_on_ground(state=state).values()
        ):
            self.done = True
        if self.early_stop_condition is not None and self.early_stop_condition(state):
            self.done = True
    ...
    def is_reset_state_valid(self, obs: Observation) -> bool:
        return super().is_reset_state_valid(obs=obs) and not any(
            TableTopFR5MujocoEnv.get_are_objs_on_ground(state=obs.state).values()
        )
    

class TableTopRobotiq85MujocoEnv(MujocoUR5Robotiq85fEnvFromObjConfigList):
    def __init__(
        self,
        table_asset_path: str = "scalingup/environment/mujoco/assets/custom/table.xml",
        table_length: float = 1.165,
    ...
    def get_state(self) -> EnvState:
        state = super().get_state()
        if self.end_episode_on_drop and any(
            TableTopRobotiq85MujocoEnv.get_are_objs_on_ground(state=state).values()
        ):
            self.done = True
        if self.early_stop_condition is not None and self.early_stop_condition(state):
            self.done = True
    ...
    def is_reset_state_valid(self, obs: Observation) -> bool:
        return super().is_reset_state_valid(obs=obs) and not any(
            TableTopRobotiq85MujocoEnv.get_are_objs_on_ground(state=obs.state).values()
        )
    

class TableTopFR5Robotiq85MujocoEnv(MujocoFR5Robotiq85fEnvFromObjConfigList):
    def __init__(
        self,
        table_asset_path: str = "scalingup/environment/mujoco/assets/custom/table.xml",
        table_length: float = 1.165,
    ...
    def get_state(self) -> EnvState:
        state = super().get_state()
        if self.end_episode_on_drop and any(
            TableTopFR5Robotiq85MujocoEnv.get_are_objs_on_ground(state=state).values()
        ):
            self.done = True
        if self.early_stop_condition is not None and self.early_stop_condition(state):
            self.done = True
    ...
    def is_reset_state_valid(self, obs: Observation) -> bool:
        return super().is_reset_state_valid(obs=obs) and not any(
            TableTopFR5Robotiq85MujocoEnv.get_are_objs_on_ground(state=obs.state).values()
        )
    

class TableTopFR5DHPGI80MujocoEnv(MujocoFR5DHPGI80EnvFromObjConfigList):
    def __init__(
        self,
        table_asset_path: str = "scalingup/environment/mujoco/assets/custom/table.xml",
        table_length: float = 1.165,
    ...
    def get_state(self) -> EnvState:
        state = super().get_state()
        if self.end_episode_on_drop and any(
            TableTopFR5DHPGI80MujocoEnv.get_are_objs_on_ground(state=state).values()
        ):
            self.done = True
        if self.early_stop_condition is not None and self.early_stop_condition(state):
            self.done = True
    ...
    def is_reset_state_valid(self, obs: Observation) -> bool:
        return super().is_reset_state_valid(obs=obs) and not any(
            TableTopFR5DHPGI80MujocoEnv.get_are_objs_on_ground(state=obs.state).values()
        )
